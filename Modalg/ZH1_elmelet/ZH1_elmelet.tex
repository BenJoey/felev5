...
\begin{enumerate}
	...
	\end{itemize}
	Mindkét esetben $x\in u_1$
	\item\textbf{Unknown defs No.1}
	\begin{itemize}
		\item $\delta_2f(a,b):=(y\mapsto f(a,y))'|_{y=b}\in\R^{n_2\times n_2}$
		\item $\delta_1f(a,b):=(x\mapsto f(x,b))'|_{x=a}\in\R^{n_1\times n_1}$
	\end{itemize}
	...
	$\varphi$ és $\xi$ is megoldás, akkor $\varphi(t)=\psi(t)\text{,  }\forall t\in D_\varphi\cap D_\xi$
