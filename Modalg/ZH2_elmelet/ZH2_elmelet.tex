...
\begin{enumerate}
	...
	$f,g\in\R_{2\cdot\pi}$ ortogonális (merőleges), ha $\int\limits_0^{2\cdot\pi}f\cdot g=0$
	...
	\frac{1}{\pi}\int\limits_0^{2\cdot\pi}f(x)\cdot\sin(k\cdot x)dx,\br(k\geq1)$
	...
	$\exists f\in C_{2\cdot \pi}$, hogy $f$ Fourier-sora divergál egy adott pontban.
\end{enumerate}
\end{document}